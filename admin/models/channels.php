...
class StreamyModelChannels extends JModelList
{
	...
	protected function getListQuery() 
	{
		...
		$query->select(
			$this->getState(
					'list.select',
					'c.id,c.name,c.width,c.height,c.url,c.chaturl,c.published,c.ordering,t.name as type'
					)
				);
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(c.name LIKE ' . $search . ' OR c.url LIKE ' . $search . ' OR c.chaturl LIKE ' . $search . ')');
			}
		}
